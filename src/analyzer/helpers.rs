...
/// Common Korean particles appended after nouns. Used by the test-only
/// RegexCounter to mirror the PHP plugin fallback; production runs lindera.
#[cfg(test)]
...
pub static SCRIPT_STYLE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?si)<(script|style)[^>]*?>.*?</(?:script|style)>").expect("유효한 정규식"));
pub static TAG: Lazy<Regex> = Lazy::new(|| Regex::new(r"<[^>]*>").expect("유효한 정규식"));
pub static WHITESPACE: Lazy<Regex> = Lazy::new(|| Regex::new(r"\s+").expect("유효한 정규식"));
pub static H2: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)<h2\b[^>]*>").expect("유효한 정규식"));
pub static H2_INNER: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?is)<h2\b[^>]*>(.*?)</h2>").expect("유효한 정규식"));
pub static P_INNER: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?is)<p\b[^>]*>(.*?)</p>").expect("유효한 정규식"));
pub static IMG: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)<img\b[^>]*>").expect("유효한 정규식"));
pub static IMG_ALT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(?i)\balt\s*=\s*"[^"]+""#).expect("유효한 정규식"));
pub static NON_ASCII: Lazy<Regex> = Lazy::new(|| Regex::new(r"[^\x00-\x7F]").expect("유효한 정규식"));
pub static A_HREF: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(?is)<a\s+[^>]*?href\s*=\s*"([^"]+)""#).expect("유효한 정규식"));
pub static SENTENCE_END: Lazy<Regex> = Lazy::new(|| Regex::new(r"[.!?。?]+\s*").expect("유효한 정규식"));
pub static UL_OL: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)<(ul|ol)\b").expect("유효한 정규식"));
...
pub static INFORMAL: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?:ㅋㅋ+|ㅎㅎ+|ㅠㅠ+|ㅜㅜ+|ㅇㅇ|ㄴㄴ|헐\b|대박\b|레알\b|개꿀\b|쩐다\b|굿굿)").expect("유효한 정규식")
});
...
pub static HEADING: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)<h([1-6])\b[^>]*>").expect("유효한 정규식")
});
